        ...
        ZStack(alignment: .bottomLeading) {
            GeometryReader { geo in
                CameraPreviewView(session: cameraService.session)
                    ...
                    .overlay(HeatmapOverlayView(cameraService: cameraService))
                    .overlay(SpotOverlayView(cameraService: cameraService))
...
private struct HeatmapOverlayView: View {
    ...
    private func heatColor(for value: CGFloat) -> Color {
        ...
        let hue = (1.0 - clamped) * 0.6
        return Color(hue: hue, saturation: 0.85, brightness: 0.95)
    }
}

private struct SpotOverlayView: View {
    @ObservedObject var cameraService: CameraService
